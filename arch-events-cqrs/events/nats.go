...
import (
	...
	"github.com/jamsmendez/cqrs/models"
...
func NewNats(url string) (*NatsEventStore, error) {
	...
	navEventStore := NatsEventStore{
		conn: conn,
	}

	return &navEventStore, nil
}
...
func (n *NatsEventStore) SubscribeCreatedFeed(ctx context.Context) (<-chan CreatedFeedMessage, error) {
	createdFeedMsg := CreatedFeedMessage{}
	n.feedCreatedChan = make(chan CreatedFeedMessage, 64)
	ch := make(chan *nats.Msg, 64)

	var err error

	n.feedCreatedSub, err = n.conn.ChanSubscribe(createdFeedMsg.Type(), ch)
	if err != nil {
		return nil, err
	}
	...
	go func() {
		for {
			select {
			case msg := <-ch:
				n.decodeMessage(msg.Data, &createdFeedMsg)
				n.feedCreatedChan <- createdFeedMsg
			}
		}
	}()

	// n.feedCreatedChan es un canal bidireccional al castear con (<-chan CreatedFeedMessage)
	// se vuelve unidireccional que solo recibe
	res := (<-chan CreatedFeedMessage)(n.feedCreatedChan)
